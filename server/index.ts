...
app.post('/api/accounts', async c => {
  ...
  ).run(body.id, body.name, body.type, body.owner, body.seedBalance, body.seedDate, body.rate, body.amortizing ? 1 : 0)
  return c.json(body, 201)
})

app.get('/api/accounts/:id', c => {
  const row = db.prepare('SELECT * FROM accounts WHERE id = ?').get(c.req.param('id')) as Record<string, unknown> | null
...
app.get('/api/external-parties', c => {
  const rows = db.prepare('SELECT * FROM external_parties').all() as Record<string, unknown>[]
  return c.json(rows.map(rowToParty))
})

app.post('/api/external-parties', async c => {
  const body = await c.req.json<ExternalParty>()
  db.prepare('INSERT INTO external_parties (id, name) VALUES (?, ?)').run(body.id, body.name)
  return c.json(body, 201)
})

app.get('/api/external-parties/:id', c => {
  const row = db.prepare('SELECT * FROM external_parties WHERE id = ?').get(c.req.param('id')) as Record<string, unknown> | null
  if (!row) return c.json({ error: 'Not found' }, 404)
...
app.delete('/api/external-parties/:id', c => {
  db.prepare('DELETE FROM external_parties WHERE id = ?').run(c.req.param('id'))
  return c.body(null, 204)
})
...
app.post('/api/schedules', async c => {
  ...
  ).run(body.id, body.sourceId, body.destinationId, body.amount, body.estimated ? 1 : 0, body.frequency, body.startDate, body.endDate ?? null, body.terminateAtZero ? 1 : 0)
  return c.json(body, 201)
})

app.get('/api/schedules/:id', c => {
  const row = db.prepare('SELECT * FROM schedules WHERE id = ?').get(c.req.param('id')) as Record<string, unknown> | null
  if (!row) return c.json({ error: 'Not found' }, 404)
...
app.get('/api/adjustments', c => {
  const rows = db.prepare('SELECT * FROM adjustments').all() as Record<string, unknown>[]
  return c.json(rows.map(rowToAdjustment))
})

app.post('/api/adjustments', async c => {
  const body = await c.req.json<Adjustment>()
  db.prepare('INSERT INTO adjustments (id, account_id, date, actual_balance) VALUES (?, ?, ?, ?)').run(
    body.id, body.accountId, body.date, body.actualBalance,
  )
  return c.json(body, 201)
})
...
app.get('/api/scenarios/:id', c => {
  const row = db.prepare('SELECT * FROM scenarios WHERE id = ?').get(c.req.param('id')) as Record<string, unknown> | null
  if (!row) return c.json({ error: 'Not found' }, 404)
...
app.get('/api/projection', async c => {
  ...
  const accounts = (db.prepare('SELECT * FROM accounts').all() as Record<string, unknown>[]).map(rowToAccount)
  const externalParties = (db.prepare('SELECT * FROM external_parties').all() as Record<string, unknown>[]).map(rowToParty)
  const schedules = (db.prepare('SELECT * FROM schedules').all() as Record<string, unknown>[]).map(rowToSchedule)
  const adjustments = (db.prepare('SELECT * FROM adjustments').all() as Record<string, unknown>[]).map(rowToAdjustment)
  ...
  const result = project({ accounts, externalParties, schedules, adjustments: noAdj ? [] : adjustments, scenario, startDate, endDate })
