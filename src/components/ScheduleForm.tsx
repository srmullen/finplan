...
export default function ScheduleForm({ initial, accounts, externalParties, onSave, onCancel }: Props) {
  ...
  const [destinationId, setDestinationId] = useState(initial?.destinationId ?? firstNodeId)
  const [amount, setAmount] = useState(String(initial?.amount ?? ''))
  const [estimated, setEstimated] = useState(initial?.estimated ?? false)
  const [frequency, setFrequency] = useState<Frequency>(initial?.frequency ?? 'monthly')
  const [startDate, setStartDate] = useState(initial?.startDate ?? today)
  const [endDate, setEndDate] = useState(initial?.endDate ?? '')
  const [terminateAtZero, setTerminateAtZero] = useState(initial?.terminateAtZero ?? false)
