...
describe("AccountForm — add mode (no initial)", () => {
	...
	it("calls onSave with a generated id and default values on submit", () => {
		...
		fireEvent.submit(
			screen.getByRole("button", { name: "Add account" }).closest("form")!,
		);
	...
	it("parses non-zero seedBalance and rate on submit", () => {
		...
		fireEvent.change(screen.getByLabelText("Seed balance ($)"), {
			target: { value: "10000" },
		});
	...
	it("sets institution when provided", () => {
		...
		expect(onSave).toHaveBeenCalledWith(
			expect.objectContaining({ institution: "Chase" }),
		);
	});
...
describe("AccountForm — edit mode (with initial)", () => {
	...
	it("pre-fills institution when initial has one", () => {
		render(
			<AccountForm
				{...defaultProps}
				initial={{ ...existingAccount, institution: "Navy Federal" }}
			/>,
		);
		expect(
			(screen.getByLabelText("Institution") as HTMLInputElement).value,
		).toBe("Navy Federal");
	});
	...
	it("toggles the amortizing checkbox", () => {
		...
		expect(onSave).toHaveBeenCalledWith(
			expect.objectContaining({ amortizing: false }),
		);
	});
	...
	it("updates owner when owner select is changed", () => {
		...
		fireEvent.change(screen.getByLabelText("Name"), {
			target: { value: "Her Account" },
		});
		fireEvent.submit(
			screen.getByRole("button", { name: "Add account" }).closest("form")!,
		);
		expect(onSave).toHaveBeenCalledWith(
			expect.objectContaining({ owner: "Wife" }),
		);
	});
	...
	it("updates seed date when date input is changed", () => {
		...
		fireEvent.change(screen.getByLabelText("Name"), {
			target: { value: "Dated Account" },
		});
		fireEvent.submit(
			screen.getByRole("button", { name: "Add account" }).closest("form")!,
		);
		expect(onSave).toHaveBeenCalledWith(
			expect.objectContaining({ seedDate: "2025-06-01" }),
		);
	});
});
